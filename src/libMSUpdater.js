...
function MSUpdate(updObj) { //<represents update
	...
	this.cves = this._collection2Arr(this.updateObj.CveIDs);
	this.urls = [this.updateObj.SupportUrl].concat(this._collection2Arr(this.updateObj.MoreInfoUrls));
}
...
MSUpdate.prototype._collection2Arr = function (coll) {
	var arr = [];
...
function MSUpdater() { //<represents MS Update service, singleton
	this.session = new ActiveXObject("Microsoft.Update.Session");
	this.installer = this.session.CreateUpdateInstaller();
	this.downloader = this.session.CreateUpdateDownloader();
	this.searcher = this.session.CreateUpdateSearcher();
...
MSUpdater.prototype.getUpdates = function (criteria, online) { //<givess you updates satisfying the criteria, see https://msdn.microsoft.com/en-us/library/aa386526.aspx to understand query language
	if(typeof online=="undefined")online=true;
	this.searcher.Online=online;
